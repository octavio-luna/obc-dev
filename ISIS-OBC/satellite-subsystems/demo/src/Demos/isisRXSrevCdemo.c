...
static Boolean getRXSTelemetryTest(void)
{
	...
	eng_value = ((float)telemetryValue) * (-0.5) - 49;
	printf("Receiver RSSI = %f dBm\r\n", eng_value);

	telemetryValue = telemetry.fields.voltage;
	eng_value = ((float)telemetryValue) * 0.004912686 + 0.05;
...
{
	isis_rxs_c__get_frame__from_t frame;
	driver_error_t error;
	unsigned int i;

	frame.data = malloc(SIZE_RXFRAME);
	memset(frame.data, 0, SIZE_RXFRAME);

	error = isis_rxs_c__get_frame(0, &frame);

	if(error != driver_error_none)
	{
		printf("\r\nRXS get frame error: %d\r\n", (int)error);
	}
	else
	{
		printf("\r\n\t Frame Length: %hu", frame.length);\
		printf("\r\n\t Frame Doppler: %d", (int)frame.doppler);
		printf("\r\n\t Frame rssi: %d\r\n", (int)frame.rssi);
	...
	else
	{
		...
		else
		{
			...
			for(i = 0; i < _RXS_UART_BUFFERSIZE; i++)
			{
				...
				if(data[i] == _RXS_UART_PILOT_BYTE && pilot_byte < 0)
				{
					pilot_byte = i;
					break;
				}
			...
			if(pilot_byte >= 0)
			{
				 ...
				 * crc16 - 2 bytes
				 */
				isis_rxs_c__get_frame__from_t *raw_frame_ptr = (void*)&data[pilot_byte+1];
				...
				if(pilot_byte + 7 + raw_frame_ptr->length + 2 > _RXS_UART_BUFFERSIZE)
				{
				...
				for(i = 0; i < raw_frame_ptr->length; i++)
				{
					...
					printf(" (0x%02X) ", data[i+pilot_byte+7]);
				...
				memcpy(&uartframe_crc, &data[pilot_byte + 7 + raw_frame_ptr->length], sizeof(uartframe_crc));
				uartframe_crc = (uartframe_crc >> 8) | (uartframe_crc << 8); // swap bytes, this CRC16 is MSB

				if(uartframe_crc == checksum_calculateCRC16LUT((void*)&data[pilot_byte+1], raw_frame_ptr->length + 6, LUT_TCCRC, CRC16_DEFAULT_STARTREMAINDER, TRUE))
				{
...
static Boolean selectAndExecuteRXSDemoTest(void)
{
	...
	printf("\t 9) Get and RX frame (i2c) \n\r");
	printf("\t 10) Get an RX frame over UART (This is blocking) \r\n");
