...
#ifdef DEBUG
	#define DEBUGSEQ csp_print("%s %u\n",__FUNCTION__,__LINE__)
	#define DEBUGF(...) csp_print(__VA_ARGS__)
#else
	#define DEBUGSEQ
	#define DEBUGF(...)
#endif
...
DEBUGSEQ; DEBUGF("input.iface->addr=%u packet->id.dst=%u to_me=%d\n",input.iface->addr,packet->id.dst,is_to_me);
	/* Deduplication */
