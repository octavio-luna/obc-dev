...
#if CSP_DEBUG_CSP_PACKETS
	#define csp_hex_debug_dump(...) csp_hex_dump(__VA_ARGS__)
#else
	#define csp_hex_debug_dump(...) 
#endif
...
static int csp_if_tun_tx(csp_iface_t * iface, uint16_t via, csp_packet_t * packet, int from_me) {
	...
	if (packet->id.dst == ifconf->tun_src) {
		 ...
		 */
		csp_hex_debug_dump("incoming packet", packet->data, packet->length);
		...
		csp_hex_debug_dump("new frame", new_packet->frame_begin, new_packet->frame_length + 16);

		csp_id_strip(new_packet);

		csp_hex_debug_dump("new packet", new_packet->data, new_packet->length);
		...
		csp_hex_debug_dump("packet", packet->data, packet->length);

		/* Apply CSP header */
		csp_id_prepend(packet);

		csp_hex_debug_dump("frame", packet->frame_begin, packet->frame_length);
		...
		csp_hex_debug_dump("new frame", new_packet->frame_begin, new_packet->frame_length);
