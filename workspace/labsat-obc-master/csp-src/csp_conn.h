...
// Old versions of Newlib in the toolchain have no stdatomic.h file
// #include <stdatomic.h>
...
struct csp_conn_s {
	...
	int type; // Old gcc versions do not support _Atomic(int)
	// atomic_int state; /* Connection state (CONN_OPEN or CONN_CLOSED) */
	int state; // Old gcc versions do not support _Atomic(int)
	...
	/* in this freertos version we do not have static queue creation 
