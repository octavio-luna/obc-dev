...
//////////////////////////////////////////////////////////////////////////////
// these are not defined in old newlib versions
unsigned int strnlen(const char* p, unsigned int n) {
	const char *q=p, *end=p+n;
	while( q<end ) {
		if( ! *q ) return q-p;
		++q;
	}
...
char * strtok_r(register char *__restrict s , register const char *__restrict delim , char **__restrict lasts) {
