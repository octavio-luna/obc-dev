...
static void csp_yaml_end_if(struct data_s * data, unsigned int * dfl_addr) {
    ...
    if (strcmp(data->driver, "kiss") == 0) {
		...
		iface = pvPortMalloc(sizeof(csp_iface_t));
		csp_if_tun_conf_t * ifconf = pvPortMalloc(sizeof(csp_if_tun_conf_t));
		...
		iface = pvPortMalloc(sizeof(csp_iface_t));
		csp_if_udp_conf_t * udp_conf = pvPortMalloc(sizeof(csp_if_udp_conf_t));
...
void csp_yaml_init(char * filename, unsigned int * dfl_addr) {
	...
	vPortFree(data.name);
	vPortFree(data.driver);
	vPortFree(data.device);
	vPortFree(data.addr);
	vPortFree(data.netmask);
	vPortFree(data.server);
	vPortFree(data.is_dfl);
	vPortFree(data.baudrate);
	vPortFree(data.source);
	vPortFree(data.destination);
	vPortFree(data.listen_port);
	vPortFree(data.remote_port);
	vPortFree(data.promisc);
